...
function showText(){
    ...
    let points1 = font.textToPoints("w" ,135,90,fontSize);
    ...
    // converts font into an object and converts font to points.
   let points2 = font.textToPoints("e" ,145,190,fontSize);
    for(let i=0; i<points2.length;i++) {
        pt = points2[i];
        vehicle = new Vehicle(pt.x, pt.y);
        line2.push(vehicle);
    }
    let points3 = font.textToPoints("l" ,160,325,fontSize);
    for(let i=0; i<points3.length;i++) {
        pt = points3[i];
        vehicle = new Vehicle(pt.x, pt.y);
        line3.push(vehicle);
    }
    let points4 = font.textToPoints("c" ,145,425,fontSize);
    for(let i=0; i<points4.length;i++) {
        pt = points4[i];
        vehicle = new Vehicle(pt.x, pt.y);
        line3.push(vehicle);
    }
    let points5 = font.textToPoints("o" ,145,525,fontSize);
    for(let i=0; i<points5.length;i++) {
        pt = points5[i];
        vehicle = new Vehicle(pt.x, pt.y);
        line3.push(vehicle);
    }
    let points6 = font.textToPoints("m" ,135,625,fontSize);
    for(let i=0; i<points6.length;i++) {
        pt = points6[i];
        vehicle = new Vehicle(pt.x, pt.y);
        line3.push(vehicle);
    }
    let points7 = font.textToPoints("e" ,145,725,fontSize);
    for(let i=0; i<points7.length;i++) {
        pt = points7[i];
        vehicle = new Vehicle(pt.x, pt.y);
        line3.push(vehicle);
    }
    let points8 = font.textToPoints("!" ,160,825,fontSize);
